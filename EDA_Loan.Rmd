...
```{r}
ggplot(aes(y = EmploymentStatusDuration / 12, x = EstimatedEffectiveYield, 
           color = ProsperScore), 
       data = subset(loanData, !is.na(loanData$ProsperScore))) +
...
home.loans$ProsperRating..Alpha. <- factor(home.loans$ProsperRating..Alpha., ordered = T,
...
```{r}
a <- loanData %>%
    filter(ListingCategory..numeric. == 2, LoanStatus == 'Cancelled')
...
dubious.borrowers <- loanData %>%
    ...
    summarise(n = n()) %>%
    mutate(freq = round(n / sum(n) * 100, 2))
dubious.borrowers$ProsperRating..Alpha. <- ordered(dubious.borrowers$ProsperRating..Alpha., 
                                                    levels = c('AA', 'A', 'B', 'C', 'D', 'E', 'HR'))
...
ggplot(aes(x = ProsperRating..Alpha., y = freq, fill = freq), 
    ...
    geom_text(aes(label = sprintf("%2.1f%%", round(freq, 2))), 
              ...
              nudge_x = 0.06) +
    theme(axis.text.x = element_text(face = 'bold.italic', colour = 'dodgerblue4', size = 15),
          axis.text.y=element_blank(),
          axis.title.x = element_text(face = 'bold', colour = 'dodgerblue4', size = 16),
          axis.title.y = element_text(face = 'bold', colour = 'dodgerblue4', size = 16))
...
base <- loanData %>%
    filter(StatedMonthlyIncome != 0.0, MonthlyLoanPayment != 0.0, ProsperRating..Alpha. != "") %>%
    ggplot(aes(y = StatedMonthlyIncome, x = MonthlyLoanPayment)) +
    geom_jitter(color = 'black', fill = '#F79420', shape = 21) +
    coord_cartesian(ylim = c(100, 30000), xlim = c(10, 1000)) +
    scale_y_continuous(trans = log10_trans(), breaks = c(100, 1000, 10000, 20000, 30000)) +
    scale_x_continuous(trans = log10_trans(), breaks = c(10, 50, 100, 200, 500, 700, 1000)) +
    theme_pander()
scatter1 <- base + geom_smooth(span = 0.3, method = 'lm') + 
    theme(plot.margin=unit(c(0, 1, 0, 0), "cm")) + xlab('\nAmount paid/month for loan')
scatter2 <- base + geom_smooth(span = 0.3, method = 'lm', formula = y ~ splines::bs(x, 3)) + 
...
Here I am trying to see the relationship of _MonthlyLoanPayment_ with _StatedMonthlyIncome._ We can see from the above statterplot that most of the _MonthlyLoanPayment_ is distribued from 10 to 1000 and the _StatedMonthlyIncome_ is distributed from 100 to 30000. Both the scales are transformed in log scale and we can clearly observe two things, namely,
    ...
    1. There is defineite a strong positive correlation between monthly income and monthly loan amount.
